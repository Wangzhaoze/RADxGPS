...
def train_net(net, device, data_path, epochs=30, batch_size=1, lr=0.00001):
    ...
    log_dir = "./logs/ogm2traj_densenet201"  # 指定保存日志的目录
        ...
        for epoch in range(epochs):
            ...
            for batch_index, (image, label) in enumerate(train_loader):
                ...
                if loss.item() < best_loss:
                    best_loss = loss.item()
                    torch.save(net.state_dict(), './pth/best_model_ogm2traj_k7_densenet.pth')
...
if __name__ == "__main__":
    ...
    print("------training------")
    train_net(net, device, data_path, epochs=60, batch_size=1)
