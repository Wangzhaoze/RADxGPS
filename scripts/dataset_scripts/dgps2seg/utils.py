...
def Time_Stamp_Synchronization(long_dict, short_dict):
    ...
    for short_idx in range(len(short_time_list)):

        for long_idx in range(long_start, len(long_time_list)-1):
            ...
            long_time_list[long_idx+1] > short_time_list[short_idx]:
                ...
                long_start = long_idx
                new_short_index.append(short_idx)
                break
    ...
    for key in short_dict.keys():
        new_short_dict[key] = short_dict[key][new_short_index]

    new_long_dict.update(new_short_dict)

    return new_long_dict, new_short_dict


############## find original row-index of car-center in rotated image ################
def find_line(img1, img2) -> Any:
...
def post_process(seg_img):
    ...
    res = cv2.dilate(edge, kernel, iterations=1)
    return res
